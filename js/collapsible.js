...
setTimeout(()=>{
  const elem_remove=document.querySelector(".hero-text1");
  const elem_add=document.querySelector('.hero-text2');
  ...
  const Liquidliq_btn = class Liquidliq_btn {
    constructor(liq_btn) {
      const { width, height } = liq_btn.getBoundingClientRect();
      const liq_btnStyles = window.getComputedStyle(liq_btn);
      console.log()
      const options = liq_btn.dataset || {};
  
      this.font = `${liq_btnStyles['font-size']} ${liq_btnStyles['font-family']}`;
      ...
      this.padding = parseFloat(liq_btnStyles.paddingRight);
      ...
      this.color2 = options.color2 || '#8F17E1';
      this.color3 = options.color3 || '#E509E6';
      this.textColor = liq_btnStyles.color || '#FFFFFF';
      this.layers = [{
        points: [],
        viscosity: 0.5,
        mouseForce: 100,
      ...
      // }
      liq_btn.append(this.canvas)
      this.touches = [];
      this.noise = options.noise || 0;
      liq_btn.addEventListener('mousemove', this.mousemove);
      liq_btn.addEventListener('mouseout', this.mouseout);
      this.initOrigins();
      this.animate();
      this.restingFace();
    }
    ...
    restingFace() {
      // force a mouse move on each liq_btn
  ...
  const liq_btns = document.querySelectorAll('.liq_btn');
  
  liq_btns.forEach(liq_btn => {
    liq_btn.liquidliq_btn = new Liquidliq_btn(liq_btn);
  })
